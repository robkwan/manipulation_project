...
class ObjectDetection(Node):
    ...
    def callback(self, msg: PointCloud2) -> None:
        ...
        try:
            ...
            self.get_logger().info(f"PointCloud2 msg width {msg.width} x height {msg.height}.")

            # Filtered cloud for surface detection
            filtered_cloud_plane = self.filter_cloud(cloud, max_x_dist=0.8, min_y_dist=-0.3, min_height=-0.5, max_height=0.0)
            # Filtered cloud for object detection
            filtered_cloud_objects = self.filter_cloud(cloud, max_x_dist=0.8, min_y_dist=-0.3, min_height=0.0, max_height=0.5)
    ...
    def filter_cloud(self, cloud: pcl.PointCloud, max_x_dist: float, min_y_dist: float, min_height: float, max_height: float) -> Union[pcl.PointCloud, None]:
        ...
        try:
            ...
            for i in range(cloud.size):
                point = cloud[i]
                if (point[0] <= max_x_dist) and (point[1] > min_y_dist) and (min_height <= point[2] <= max_height):
                    indices.append(i)
    ...
    def extract_plane(self, cloud: pcl.PointCloud) -> Tuple[np.ndarray, np.ndarray, pcl.PointCloud]:
        ...
        seg.set_distance_threshold(0.1) #0.01
    ...
    def extract_clusters(self, cloud: pcl.PointCloud, cluster_type: str) -> Tuple[List[pcl.PointCloud], List[List[float]], List[List[float]]]:
        ...
        ec.set_MinClusterSize(1000) #100
        ec.set_MaxClusterSize(500000) #80000
